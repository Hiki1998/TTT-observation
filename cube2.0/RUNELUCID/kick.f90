      ...
      do while(ip1/=0) ! particle A
        ...
        do while (ip2/=0) ! particle B in the same cell
          ...
          if (0.<rmag .and. rmag<pp_range) then
            ...
            ! if (maxval(fpp) > 20) then 
            !   print*, 'fpp>10',ip1,ip2,rmag,fpp
            !   print*, 'pp_range',pp_range
            !   print*, 'app',app
            !   print*, 'F_ra(rmag,app)',F_ra(rmag,app)
            !   print*, 'F_ra(rmag,pp_range)',F_ra(rmag,pp_range)
            !   print*, 'x1',xf(:,ip1)
            !   print*, 'x2',xf(:,ip2)
            !   ! stop
            ! endif
            af(:,ip1)=af(:,ip1)+fpp
            af(:,ip2)=af(:,ip2)-fpp
            ! if (max(sum(af(:,ip1)**2),sum(af(:,ip2)**2)) > 1e3) then 
            !   print*, 'fpp2>1e4',ip1,ip2,rmag,fpp
            !   print*, 'pp_range',pp_range
            !   print*, 'app',app
            !   print*, 'F_ra(rmag,app)',F_ra(rmag,app)
            !   print*, 'F_ra(rmag,pp_range)',F_ra(rmag,pp_range)
            ...
            do i_neighbor=1,n_neighbor ! neighbor cells
              ...
              do while (ip2/=0) ! particle B
                ...
                if (rmag<pp_range) then
                  ...
                  af(:,ip1)=af(:,ip1)+fpp
                  af(:,ip2)=af(:,ip2)-fpp
                  ! if (max(sum(af(:,ip1)**2),sum(af(:,ip2)**2)) > 1e4) then 
                  !   print*, 'fpp2>1e4',ip1,ip2,rmag,fpp
                  !   print*, 'pp_range',pp_range
                  !   print*, 'app',app
                  !   print*, 'F_ra(rmag,app)',F_ra(rmag,app)
